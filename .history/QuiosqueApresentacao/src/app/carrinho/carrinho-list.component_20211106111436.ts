...
export class CarrinhoListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.carrinhoService.read().subscribe(carrinhos => {
      ...
      .filter((carrinho: Carrinho) => carrinho.produto.name.toLocaleLowerCase().indexOf(this._filterBy.toLocaleLowerCase()) > -1)                                             .filter((carrinho: Carrinho) => carrinho.enviadoPedido !== true);
  ...
  set filter(value: string) {
    ...
    this.filteredCarrinhos =
      this.carrinhos
      .filter((carrinho: Carrinho) => carrinho.telefone.toString().indexOf(this._filterBy.toString()) > -1)
