...
export class CarrinhoListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.carrinhoService.read().subscribe(carrinhos => {
      ...
      console.log('filtro init ' + this._filterBy);
  ...
  set filter(value: string) {
    ...
    this.filteredCarrinhos =
      ...
      .filter((carrinho: Carrinho) => carrinho.enviadoPedido !== true)
      .filter((carrinho: Carrinho) => carrinho.telefone === environment.telefone)
      .filter((carrinho: Carrinho) => carrinho.produto.name.toString().indexOf(this._filterBy.toString()) > -1);
      filter((produto: Produto) => produto.name.toLocaleLowerCase().indexOf(this._filterBy.toLocaleLowerCase()) > -1);
  }
