...
export class CarrinhoListComponent implements OnInit {
  ...
  pedidoCreate(carrinhoId: number): void {
    ...
    this.carrinhoService.readById(carrinhoId).subscribe(carrinho => {
      ...
      this.pedido.id = carrinho.peid;
      this.pedido.produto.id = this.produto.id;
      this.pedido.produto.name = this.produto.name;
      this.pedido.produto.imageUrl = this.produto.imageUrl;
