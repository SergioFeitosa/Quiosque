...
export class PedidoListComponent implements OnInit {
  ...
  entregaCreate(pedidoId: number): void {
    ...
    openPopup(pedidoId: number): void {
      ...
      console.log('open1 ' + pedidoId);

      // tslint:disable-next-line:no-unused-expression
      this.pedidoService.readById(pedidoId).subscribe(pedido => {
        this.pedido = pedido;
        console.log('open2 ' + pedido.observacao);
        console.log('open3 ' + this.pedido.observacao);
    ...
    // tslint:disable-next-line:typedef
    closePopup(pedidoId: number) {

      console.log('saida0 ' + this.pedido.observacao);
      this.pedidoObservacao = this.pedido.observacao;
      console.log('saida01 ' + this.pedidoObservacao);
      ...
      this.pedidoService.readById(pedidoId).subscribe(pedido => {
        ...
        console.log('ler close ' + pedidoId);

        console.log('saida1 ' + this.pedido.observacao);

        pedido.observacao = this.pedidoObservacao;

        console.log('saida2 ' + pedido.observacao);

        this.pedidoService.update(thispedido).subscribe(() => {
          this.pedidoService.showMessage('Produto Atualizado');
          console.log('saida3 ' + this.pedido.observacao);
        });

        console.log('saida4 ' + pedido);
