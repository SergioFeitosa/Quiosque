...
export class PedidoListComponent implements OnInit {
  ...
  set filter(value: string) {
    ...
    this.filteredPedidos =
      this.pedidos
      .filter((pedido: Pedido) => pedido.entrega !== true)
      .filter((pedido: Pedido) => carrinho.telefone === environment.telefone)
      .filter((pedido: Pedido) => carrinho.produto.name.toLocaleLowerCase().indexOf(this._filterBy.toLocaleLowerCase()) > -1);
      .filter((pedido: Pedido) => pedido.telefone.toString().indexOf(this._filterBy.toString()) > -1);
    }
