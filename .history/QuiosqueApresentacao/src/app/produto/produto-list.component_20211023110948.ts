import { PipeNamePipe } from './../../../../.history/QuiosqueApresentacao/src/app/pipe/replace.pipe_20211016170241';
...
export class ProdutoListComponent implements OnInit {
  ...
  openPopup(produtoId: number): void {
    ...
    this._produtos = this.produtoService.retrieveAll().
    filter((produto: Produto) => produto.id === this.produtoId);
    this.produto = this._produtos[1];

    this.displayStyle = 'block';
    this.produtoId = produtoId;
    this.produtoName = produto.PipeNamePipe;
    console.log('passei pelo open ' + this.produto.id);
    console.log('passei pelo open ' + this.produto.name);
  }
  // tslint:disable-next-line:typedef
  closePopup() {
