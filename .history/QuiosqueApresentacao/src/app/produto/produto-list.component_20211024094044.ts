...
export class ProdutoListComponent implements OnInit {
  ...
  openPopup(produtoId: number): void {
    ...
    this.produtoService.readById(produtoId).subscribe(produto => { this.produto = produto; console.log(produto.name); });

    console.log('nome ' + this.produto.name);

    this.produtoName = this.produto.name;
    this.produtoImageUrl = this._produto.imageUrl;
    this.produtoDescricao = this._produto.description;
    this.produtoPreco = this._produto.price;
    this.produtoTempoPreparacao = this._produto.preparationTime;
    this.produtoAvaliacao = this._produto.rating;
