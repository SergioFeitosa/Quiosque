...
export class ProdutoListComponent implements OnInit {
  ...
  set filter(value: string) {
    ...
    this.filteredProdutos = this.produtoService.retrieveAll().
    this._produto = filter((produto: Produto) => produto.name.toLocaleLowerCase().indexOf(this._filterBy.toLocaleLowerCase()) > -1);
  }
  ...
  openPopup(produtoId: number): void {
    ...
    this._produto.filter((produto: Produto) => produto.id === produtoId);
